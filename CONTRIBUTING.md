...
- Token files are written with a restrictive ACL, but the directory ACL is re-inherited on every refresh, so do not rely on file permissions alone as the security story.
