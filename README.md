...
Read-only (`accounting.reports.trialbalance.read`, the granular scope new Xero apps require); this tool cannot write to any ledger. `token.json` and `.env` are gitignored. They are credentials, so treat them like passwords. The process lock prevents concurrent rotation but does **not** encrypt either file: both remain plaintext in the clone directory and inherit its Windows permissions. Keep the clone inside your own user profile; a clone under a shared path like `C:\Tools` hands the live refresh token and client secret to every local user. On a shared machine, strip the inherited access on the clone directory itself before scheduling anything, with `icacls <clone-dir> /inheritance:r /grant:r <your-username>:(OI)(CI)F`, because every token refresh rewrites `token.json` as a new file that re-inherits the directory's permissions, so hardening the file alone lasts until the next run. `token.json.lock` contains only zero-valued lock bytes and no credentials. MIT-licensed utility code, no warranty; outputs feed professional review like any other workpaper input. Not affiliated with or endorsed by Xero.
